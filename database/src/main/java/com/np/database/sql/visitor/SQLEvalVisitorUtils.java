...
import com.np.database.sql.dialect.mysql.visitor.MySqlEvalVisitorImpl;
import com.np.database.sql.dialect.oracle.visitor.OracleEvalVisitor;
import com.np.database.sql.dialect.postgresql.visitor.PGEvalVisitor;
...
import com.np.database.sql.visitor.functions.*;
import com.np.database.exception.NpDbException;
import com.np.database.sql.NpSqlHelper;
import com.np.database.sql.ast.SQLExpr;
import com.np.database.sql.ast.SQLObject;
...
import static com.np.database.sql.visitor.SQLEvalVisitor.*;
...
public class SQLEvalVisitorUtils {
        ...
        if (sqlObject instanceof SQLValuableExpr) {
            ...
            if (value == null) {
                if (throwError && !sqlObject.getAttributes().containsKey(EVAL_VALUE)) {
                    throw new NpDbException("eval error : " + NpSqlHelper.toSQLString(sqlObject, dbType));
                }
            }
        }
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLHexExpr x) {
        String hex = x.getHex();
        byte[] bytes = NpHexBin.decode(hex);
        if (bytes == null) {
    ...
    public static Integer castToInteger(Object val) {
        ...
        throw new NpDbException("cast error");
    }
    ...
    public static Date castToDate(Object val) {
        ...
        throw new NpDbException("can cast to date");
    }
    ...
    public static Date castToDate(String text) {
        ...
        try {
            ...
            throw new NpDbException("format : " + format + ", value : " + text, e);
        }
    }
