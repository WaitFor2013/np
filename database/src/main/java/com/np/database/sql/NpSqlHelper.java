...
import com.np.database.sql.dialect.h2.visitor.H2OutputVisitor;
import com.np.database.sql.dialect.h2.visitor.H2SchemaStatVisitor;
import com.np.database.sql.dialect.mysql.visitor.MySqlOutputVisitor;
import com.np.database.sql.dialect.mysql.visitor.MySqlSchemaStatVisitor;
import com.np.database.sql.dialect.oracle.visitor.OracleOutputVisitor;
import com.np.database.sql.dialect.oracle.visitor.OracleSchemaStatVisitor;
import com.np.database.sql.dialect.oracle.visitor.OracleToMySqlOutputVisitor;
...
import com.np.database.sql.util.*;
...
public class NpSqlHelper {
    ...
    public static String buildToDate(String columnName, String tableAlias, String pattern, String dbType) {
        ...
        if (JdbcConstants.MYSQL.equalsIgnoreCase(dbType)) {
            ...
            return "";
            // expand date's handle method for other orm
        }
    ...
    public static long hash(String sql, String dbType) {
        ...
        for (;;) {
            ...
            if (token == Token.ERROR) {
                return NpInterUtils.fnv_64(sql);
            }
    ...
    public static String desensitizeTable(String tableName) {
        ...
        return NpInterUtils.hex_t(hash);
    }
    ...
    public static String sort(String sql, String dbType) {
        List<SQLStatement> stmtList = NpSqlHelper.parseStatements(sql, JdbcConstants.ORACLE);
        SQLCreateTableStatement.sort(stmtList);
        return NpSqlHelper.toSQLString(stmtList, dbType);
    }
}
