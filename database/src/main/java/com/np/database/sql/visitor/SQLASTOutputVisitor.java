...
import com.np.database.sql.ast.statement.SQLCreateTriggerStatement.TriggerType;
...
import static com.np.database.sql.util.NpInterUtils.getBoolean;
...
public class SQLASTOutputVisitor extends SQLASTVisitorAdapter implements ParameterizedVisitor, PrintableVisitor {
    ...
    static {
        try {
            defaultPrintStatementAfterSemi = NpInterUtils.getBoolean(System.getProperties(), "druid.sql.output.printStatementAfterSemi"); // compatible for early versions
        } catch (Throwable ex) {
            // skip
        }
    }
    ...
    public void addTableMapping(String srcTable, String destTable) {
        ...
        if (srcTable.indexOf('.') >= 0) {
            SQLExpr expr = NpSqlHelper.toSQLExpr(srcTable, dbType);
            if (expr instanceof SQLPropertyExpr) {
                srcTable = ((SQLPropertyExpr) expr).simplify().toString();
            }
        } else {
            srcTable = NpSqlHelper.normalize(srcTable);
        }
    ...
    protected void printTableSourceExpr(SQLExpr expr) {
        ...
        if (isEnabled(VisitorFeature.OutputDesensitize)) {
            ...
            if (ident != null) {
                String desensitizeTable = NpSqlHelper.desensitizeTable(ident);
    ...
    public boolean visit(SQLCreateDatabaseStatement x) {
        print0(ucase ? "CREATE DATABASE " : "create orm ");
        if (x.isIfNotExists()) {
            print0(ucase ? "IF NOT EXISTS " : "if not exists ");
        }
        x.getName().accept(this);
    ...
    public boolean visit(SQLAlterDatabaseStatement x) {
        print0(ucase ? "ALTER DATABASE " : "alter orm ");
        x.getName().accept(this);
