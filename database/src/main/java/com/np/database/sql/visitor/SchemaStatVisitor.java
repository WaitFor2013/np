...
import com.np.database.sql.TableStat.Condition;
import com.np.database.sql.TableStat.Mode;
import com.np.database.sql.TableStat.Relationship;
import com.np.database.sql.ast.*;
import com.np.database.sql.ast.expr.*;
import com.np.database.sql.ast.statement.*;
import com.np.database.sql.dialect.mysql.ast.expr.MySqlExpr;
...
import com.np.database.sql.dialect.oracle.ast.expr.OracleExpr;
import com.np.database.sql.dialect.postgresql.visitor.PGASTVisitorAdapter;
import com.np.database.sql.repository.SchemaObject;
import com.np.database.sql.repository.SchemaRepository;
...
public class SchemaStatVisitor extends SQLASTVisitorAdapter {
    ...
    protected final Map<String, String> tablePrefixMap = new HashMap<>();
...
//        }
    ...
    public boolean visit(SQLPropertyExpr x) {
        ...
        if (tableSource instanceof SQLExprTableSource) {
            ...
            if (expr instanceof SQLIdentifierExpr) {
                ...
                //表，缩写处理
                //log.info("表名 {} ,缩写 {}", table.getName(), x.getOwner());
                tablePrefixMap.put(table.getName(), x.getOwner().toString());
    ...
    public boolean containsColumn(String tableName, String columnName) {
        ...
        if (p != -1) {
            SQLExpr owner = NpSqlHelper.toSQLExpr(tableName, dbType);
